...
The first 4 bytes of our file is an integer specifying the size of the rest of the file. We then feed in a similar script as in problem 1, where we modify the return address to point to where we placed our shellcode (after the stored eip).  



2)

It was determined that the stored eip was 148 bytes after msg. Thus, there needed to be 148 bytes of padding.
...
In order to retrieve the canary value, we needed to add the "\\x" characters 2 bytes before where the stack canary lies. Specifically, we padded the string by 12 bytes, inserted "\\" and "x", and then ended the string. Once the while loop encounters "\\x", it would skip to the stack canary in the next interation. 
...
We store our shellcode inside of the environment variables. We then inject the address of our shellcode (inside of the environment variables) 8 bytes into buf. We pad the rest of buf. We take advantage of the off by one vulnerability to modify the stored sfp value above buf to point to the 4 bytes into buf. This is possible because we only need to change the least significant byte of the previous return address to point into the buf. From there, the program will pick up the return address inside of buf and start executing the shellcode in the environment variable. 

2)

Inside of buf, we need 8 bytes of padding, 4 bytes for the modified return address pointing to the shellcode, 52 bytes more padding, and 1 overflowed extra byte that causes the ebp to point back to buf. 

It was determined through gdb that the address of buf was 0xbffff650. Thus, the last byte of the sfp had to be overflowed with the byte "/x54". Since we know that the sfp is directly above buf, we just need to pad the rest of buf with 64-12=52 bytes. 
...
The vulnerability in this program lies in the fact that the size checking occurs before the file is actually read. Therefore, we can change the file (by adding more bytes) after the the file initially passed the size check. This would allow us to introduce a buffer overflow attack. 

Similar to problems 1 and 2, we modify the stored return address to point to 4 bytes above where it is stored. The shellcode is then placed above the stored return address. 
...
Thus, we appended 21 dummy characters (accounting for 1 more byte in buf), added 4 bytes of the modified return address, and then added our 85 bytes of shellcode. In total, after the appending of our buffer overflow bytes, the total file size became 127 + 21 + 4 + 85 = 237 bytes. We made sure to tell the program to read 237 bytes from the file "hack". 
...
Using gdb, we determined that the address of the jmp *esp instruction in code was 0x8048666. 
